...
class MessageService:
    ...
    async def _verify_session_access(
        ...
        result = await db.execute(
            select(Session).where(Session.id == thread_id)
        )
        ...
        if user_id and session.user_id != user_id:
            logger.warning(
                f"Access denied: User {user_id} attempted to access session {thread_id} "
        ...
        logger.debug(f"Access granted to session {thread_id} for user {user_id}")
        return session
    ...
    async def create_message(
        ...
        # await MessageService._verify_session_access(db, thread_id, user_id)
        ...
        logger.info(
            f"Created encrypted {role} message for thread {thread_id} "
    ...
    async def get_thread_messages(
        ...
        """
        Get all messages for a specific thread with access control.
        ...
        logger.info(
            f"Retrieved {len(messages)} encrypted messages for thread {thread_id} "
            f"(user: {user_id})"
        )
        return messages
    ...
    async def get_message_count(
        ...
        """
        Get count of messages for a thread with access control.
